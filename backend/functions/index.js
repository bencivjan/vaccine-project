...
FROM person p LEFT OUTER JOIN account_information a on p.person_id = a.person_id LEFT OUTER JOIN received_at ra on p.person_id = ra.person_id
LEFT OUTER JOIN locations l on ra.location_id = l.location_id LEFT OUTER JOIN user_login ul on a.username = ul.username
LEFT OUTER JOIN received_dose rd on p.person_id = rd.person_id LEFT OUTER JOIN vaccines v on rd.vaccine_id = v.vaccine_id
WHERE a.username = ?`;
